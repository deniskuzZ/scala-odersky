...
class FunSetSuite extends FunSuite {
  ...
  test("diff contains all elements of `s1` that are not in `s2`") {
    new TestSets {
  ...
  test("forall returns whether all bounded integers within `s` satisfy `p`") {
    new TestSets {
      val a = forall(union(s1, s2), { x: Int => x < 3 })
      assert(a===true, "forall 1")

      val b = forall(union(s1, s2), { x: Int => x < 2 })
      assert(b===false, "forall 2")
    }
  }

  test("exists returns whether there is a bounded integer within `s`") {
    new TestSets {
      val a = exists(union(s1, s2), { x: Int => x > 1 })
      assert(a===true, "exists 1")

      val b = exists(union(s1, s2), { x: Int => x > 2 })
      assert(b===false, "exists 2")
    }
  }

  test("map returns a set transformed by applying `f` to each element of `s`") {
    new TestSets {
      val s = map(union(s1, s2), { x: Int => x *2 })
