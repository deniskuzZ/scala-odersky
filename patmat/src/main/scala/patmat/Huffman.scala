...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case x :: y :: rest => (makeCodeTree(x, y) :: rest).sortBy(weight)
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeBits(subtree: CodeTree, bits: List[Bit]): List[Char] = subtree match {
      case Leaf(c, _) => if (bits.isEmpty) List(c) else c :: decodeBits(tree, bits)
      case Fork(left, right, _, _) => if (bits.head == 0) decodeBits(left, bits.tail) else decodeBits(right, bits.tail)
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeChar(tree: CodeTree)(char: Char): List[Bit] = tree match {
      case Leaf(_, _) => List()
      case Fork(left, right, _, _) => if (chars(left).contains(char)) 0 :: encodeChar(left)(char) else 1 :: encodeChar(right)(char)
    }
